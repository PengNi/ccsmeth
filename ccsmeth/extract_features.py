...
# def split_inputreads_by_holebatch(inputfile, args):
...
#     sys.stderr.write("[extract_features_hifi]generate {} read batches({})\n".format(len(holebatches),
#                                                                                     args.holes_batch))
#     return holebatches
#
#
# def worker_read_split_holebatches_to_queue(holebatches, holebatch_q, args):
#     with tqdm(total=len(holebatches),
#               desc="readbatch_handler: ", bar_format=
#               "{l_bar}{bar} [ Estimated time left: {remaining} ]") as pbar:
#         count = 0
#         while True:
#             if count >= len(holebatches):
#                 break
#             if holebatch_q.qsize() < args.threads:
#                 holebatch_q.put(holebatches[count])
#                 count += 1
#                 pbar.update(1)
#             else:
...
def _get_necessary_items_of_a_alignedsegment(readitem):
    ...
    try:
        ...
        # sys.stderr.write("\n[WARN]read-{} has no ipd/pw values\n".format(seq_name))
    ...
    return seq_name, seq_start, seq_end, fwd_seq, fwd_qual, ref_name, ref_start, ref_end, \
        cigar_tuples, cigar_stats, flag, mapq, is_unmapped, is_secondary, is_duplicate, is_reverse, \
        tag_fi, tag_ri, tag_fp, tag_rp, tag_fn, tag_rn
...
def worker_read_split_holebatches_to_queue(inputfile, holebatch_q, args):
    ...
    sys.stderr.write("[extract_features_hifi]generate {} hole/read batches({})\n".format(len(holebatches),
                                                                                         args.holes_batch))
...
# def extract_features_from_double_strand_read(readitem, motifs, holeids_e, holeids_ne, args):
#     seq_name = readitem.query_name
...
#
#     # extract features
#     seq_seq = readitem.get_forward_sequence()
#     seq_rc = complement_seq(seq_seq)
#     seq_qual = np.array(readitem.get_forward_qualities(), dtype=int)
#     seq_qual = _normalize_signals(seq_qual, args.norm)
#     reverse = readitem.is_reverse
#     cigar_info = None  # TODO
#     if args.mode == "reference":
#         cigar_info = None
#     try:
#         ipdmean_fwd = np.array(readitem.get_tag("fi"), dtype=int)
#         # ipdmean_rev = np.flip(np.array(readitem.get_tag("ri"), dtype=int))
#         ipdmean_rev = np.array(readitem.get_tag("ri"), dtype=int)  # no need to use np.filp to reverse
#         pwmean_fwd = np.array(readitem.get_tag("fp"), dtype=int)
#         pwmean_rev = np.array(readitem.get_tag("rp"), dtype=int)
#     except KeyError:
#         sys.stderr.write("\n[WARN]read-{} miss ipd/pw values\n".format(seq_name))
#         return []
#     if len(ipdmean_fwd) != len(seq_seq) or len(pwmean_fwd) != len(seq_seq):
#         sys.stderr.write("\n[WARN]read-{} has uncomplated fwd ipd/pw values\n".format(seq_name))
#         return []
#     if len(ipdmean_rev) != len(seq_seq) or len(pwmean_rev) != len(seq_seq):
#         sys.stderr.write("\n[WARN]read-{} has uncomplated rev ipd/pw values\n".format(seq_name))
#         return []
...
#     try:
#         npass_fwd = readitem.get_tag("fn")
#         npass_rev = readitem.get_tag("rn")
#     except KeyError:
#         npass_fwd = 0
#         npass_rev = 0
...
def extract_features_from_double_strand_read(readinfo, motifs, holeids_e, holeids_ne, args):
    seq_name, seq_start, seq_end, fwd_seq, fwd_qual, ref_name, ref_start, ref_end, \
        cigar_tuples, cigar_stats, flag, mapq, is_unmapped, is_secondary, is_duplicate, is_reverse, \
        tag_fi, tag_ri, tag_fp, tag_rp, tag_fn, tag_rn = readinfo
...
# def process_one_holebatch(inputfile, holebatch, motifs, holeids_e, holeids_ne, args):
#     hole_s, hole_e = holebatch
#     inputreads = _open_inputfile(inputfile, args.mode)
#
#     feature_list = []
#     read_idx = 0
...
#             read_idx += 1
#             continue
#         elif read_idx >= hole_e:
#             break
#         features_one = extract_features_from_double_strand_read(readitem,
#                                                                 motifs, holeids_e, holeids_ne, args)
...
def process_one_holebatch(holebatch, motifs, holeids_e, holeids_ne, args):
    feature_list = []
    read_idx = 0
    total_num = 0
    failed_num = 0
...
def _features_to_str(features):
    ...
    fkmer_isd_str = ",".join([str(x) for x in fkmer_isd])
    fkmer_pm_str = ",".join([str(x) for x in fkmer_pm])
    fkmer_psd_str = ",".join([str(x) for x in fkmer_psd])
    fkmer_qual_str = ",".join([str(x) for x in fkmer_qual])
    fkmer_map_str = ",".join([str(x) for x in fkmer_map]) if fkmer_map != "." else "."

    rkmer_im_str = ",".join([str(x) for x in rkmer_im])
    rkmer_isd_str = ",".join([str(x) for x in rkmer_isd])
    rkmer_pm_str = ",".join([str(x) for x in rkmer_pm])
    rkmer_psd_str = ",".join([str(x) for x in rkmer_psd])
    rkmer_qual_str = ",".join([str(x) for x in rkmer_qual])
    rkmer_map_str = ",".join([str(x) for x in rkmer_map]) if rkmer_map != "." else "."
...
def _batch_feature_list2s(feature_list):
    ...
    for featureline in feature_list:
        ...
        fipdsds.append(np.array(kmer_ipds, dtype=np.float) if kmer_ipds != "." else 0)
        fpwms.append(np.array(kmer_pwm, dtype=np.float))
        fpwsds.append(np.array(kmer_pws, dtype=np.float) if kmer_pws != "." else 0)
        fquals.append(np.array(kmer_qual, dtype=np.float))
        fmaps.append(np.array(kmer_map, dtype=np.float) if kmer_map != "." else 0)
        ...
        ripdsds.append(np.array(kmer_ipds2, dtype=np.float) if kmer_ipds2 != "." else 0)
        rpwms.append(np.array(kmer_pwm2, dtype=np.float))
        rpwsds.append(np.array(kmer_pws2, dtype=np.float) if kmer_pws2 != "." else 0)
        rquals.append(np.array(kmer_qual2, dtype=np.float))
        rmaps.append(np.array(kmer_map2, dtype=np.float) if kmer_map2 != "." else 0)
...
def extract_hifireads_features(args):
    ...
    p_split = mp.Process(target=worker_read_split_holebatches_to_queue, args=(inputpath, holebatch_q, args))
    p_split.daemon = True
    p_split.start()
    ...
    for _ in range(nproc):
        p = mp.Process(target=worker_extract_features_from_holebatches,
                       args=(holebatch_q, features_q, motifs, holeids_e, holeids_ne, args,
                             True, False))
        p.daemon = True
        p.start()
        ps_extract.append(p)
